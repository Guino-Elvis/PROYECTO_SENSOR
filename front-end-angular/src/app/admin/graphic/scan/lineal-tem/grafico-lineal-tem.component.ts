...
@Component({
  selector: 'app-grafico-lineal-tem',
  standalone: true,
  templateUrl: './grafico-lineal-tem.component.html',
  styles: []
})
export class GraficoLinealTemComponent {
  @Input() temperatura!: Sensor;
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      if (this.temperatura?.lectura?.length) {
        this.crearGrafico(this.temperatura);
      }
    }, 0);
  }
  ...
  crearGrafico(sensor: Sensor): void {
    ...
    const data = lecturas.map(l => Number(l.valor1));
